...
*Setup instructions for a gitian build of GOLDY COIN using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the GOLDY COIN
...
- [Building GOLDY COIN](#building-goldy)
...
Clone the git repositories for goldy and gitian and then checkout the goldy version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/goldy-crypto/goldy.git
cd goldy
...
Follow the instructions in [doc/release-process.md](release-process.md) in the goldy repository
...
To build GOLDY COIN (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the goldy repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/goldy/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/goldy-crypto/goldy
...
For example:
```bash
URL=https://github.com/crowning-/goldy.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit goldy=${COMMIT} --url goldy=${URL} ../goldy/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit goldy=${COMMIT} --url goldy=${URL} ../goldy/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit goldy=${COMMIT} --url goldy=${URL} ../goldy/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/goldy-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/goldy-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/goldy-build.assert
```
...
[goldy/gitian.sigs](https://github.com/goldy-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
